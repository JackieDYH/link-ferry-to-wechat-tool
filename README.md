# link-ferry-to-wechat-tool -vue3 - 202607
# 小红书/今日头条文章 → 微信公众号 上传工具（独立版Vue3）
...
├── 打开数据文件夹.bat      ← 打开 output/ 目录（生成物都在这，方便清理）
├── README.md              ← 使用文档
├── output/                ← 🗑️ 所有生成物+缓存都在这里，整个删掉即可清理
│   ├── cache/             ← 缓存集中地（可重建）
...
程序文件只有根目录那几个（.mjs/.bat/config.json/README.md/web/）。**所有运行时生成的东西都放在 `output/` 文件夹**（下载的图片、上传存档、缓存 `output/cache/`），想清理就整个删除 `output/` 文件夹，不影响程序。也可以双击 `打开数据文件夹.bat` 查看里面有什么再手动删。
